...
class RecordObjectResource(ModelResource):
    record = fields.ToOneField('DataEntry.api.RecordResource', 'record',
...
class RightsResource(ModelResource):
    # record = fields.ToOneField('DataEntry.api.RecordResource',
    # 'record', null=True, blank=True)
...
class RecordResource(ModelResource):
    recordobject = fields.ToManyField(RecordObjectResource, 'recordobject_set',
                                      related_name='record', full=True, null=True, blank=True)
    geography = fields.ToManyField(GeographyResource, 'geography_set',
                                   related_name='record', full=True, null=True, blank=True)
